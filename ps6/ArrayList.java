...
public class ArrayList implements List {
    ...
    /**
     * reverse - modifies internals of the list so that the
     * elements are reversed. O(n) run time and O(1) memory.
     */
    public void reverse() {
        Object storage = items[0];
        int forwardcount = 0;

        for (int i = length() - 1; i > (length() / 2); i--) {
            items[forwardcount] = items[i];
            items[i] = storage;
            forwardcount++;
            storage = items[forwardcount];
        }
    }
